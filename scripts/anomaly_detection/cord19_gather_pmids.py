import csv
import json
from adeft_indra.content.pmids import get_pmids_for_entity
...
with open('../../../new_grounding_table.tsv', newline='') as csvfile:
    ...
    for row in rows:
        ...
        if grounding not in pmid_map:
            try:
                pmids = get_pmids_for_entity(ns, id_, major_topic=False)
            except Exception:
                broken.add(grounding)
            pmid_map[grounding] = pmids

with open('../../results/cord19_entity_pmids2.json', 'w') as f:
    json.dump(pmid_map, f)

with open('../../results/cord19_entity_pmids_broken2.json', 'w') as f:
